...
const articles = [
  ...
  {
    id: 2,
    title: "L'art d'équilibrer son Garba",
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          <h1 className="text-3xl font-semibold text-zinc-900 mb-2 tracking-tight">Actualités Nutritionnelles</h1>
          <p className="text-zinc-600">Découvrez nos conseils et articles sur la nutrition saine et les saveurs locales</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {articles.map((article) => (
            <article
              key={article.id}
              className="bg-white rounded-xl border border-zinc-200/80 shadow-sm overflow-hidden hover:shadow-md transition-shadow"
            >
              <div className={`p-8 bg-${article.accent}-50`}>
                ...
                <h2 className="text-2xl font-semibold text-zinc-900 mb-3 tracking-tight">{article.title}</h2>
                <p className="text-zinc-600 font-medium mb-4">{article.subtitle}</p>
              </div>
              <div className="p-8">
                <p className="text-zinc-700 leading-relaxed">{article.content}</p>
              </div>
            </article>
          ))}
        </div>
